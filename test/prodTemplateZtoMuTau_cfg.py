...
process = cms.Process('prodTemplateZtoMuTau')
...
process.kineEventReweightWplusJets = cms.EDAnalyzer("EDProducer",
    config = cms.PSet(
        pluginType = cms.string("KineEventReweightExtractor"),
        weightLookupTable = cms.PSet(
            fileName = cms.string('rfio:/castor/cern.ch/user/v/veelken/CMSSW_3_1_2/bgEstKineReweights/bgEstKineEventReweightsZtoMuTau.root'),
            meName = cms.string('DQMData/bgEstKineEventReweights/WplusJets/diTauMvis')
        ),
        variables = cms.PSet(
...
setattr(process.analyzeEventsBgEstWplusJetsEnriched, "eventWeightSource", cms.VInputTag("kineEventReweightWplusJets"))

process.p = cms.Path(
   process.producePatTupleZtoMuTauSpecific
  + process.selectZtoMuTauEvents
  + process.kineEventReweightWplusJets + process.bgEstWplusJetsEnrichedAnalysisSequence
